...
def emit_all_runtime_events(env="stage"):
    ...
    # Force restart action for crash recovery (bypass RL for demo)
    print("   -> Forcing RESTART recovery action for crash...")
    from core.rl_orchestrator_safe_execution import RLOrchestratorSafeExecution
    crash_executor = RLOrchestratorSafeExecution(env)
    crash_state = {'env': env, 'event_type': 'crash', 'service': 'app', 'dataset': 'crash_dataset.csv'}
    crash_executor._safe_execute('restart_service', crash_state)
    ...
    # Force scale action for overload recovery (bypass RL for demo)
    print("   -> Forcing SCALE recovery action for overload...")
    overload_executor = RLOrchestratorSafeExecution(env)
    overload_state = {'env': env, 'event_type': 'overload', 'service': 'app', 'dataset': 'overload_dataset.csv'}
    overload_executor._safe_execute('scale_workers', overload_state)
    ...
    if env == 'stage':
        ...
        # Force test of dangerous action through orchestrator
        from core.rl_orchestrator_safe_execution import RLOrchestratorSafeExecution
        executor = RLOrchestratorSafeExecution(env)
        ...
        # This should trigger ORCH_REFUSE with prod_safety_guard reason
        executor._test_unsafe_action('delete_production_data', critical_state)
        time.sleep(1)
